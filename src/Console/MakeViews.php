...
use Thuypd\Crudgen\Services\MakeViewsService;
    ...
    public function handle()
    {
        ...
        if(!File::isDirectory($this->pathsAndNamespacesService->getCrudgenViewsStubCustom($templateViewsDirectory)))
        {
            if($templateViewsDirectory=='default-theme')
                $this->error("Publish the default theme with: php artisan vendor:publish --provider=\"Thuypd\Crudgen\CrudgenServiceProvider\" or create your own default-theme directory here: ".$this->pathsAndNamespacesService->getCrudgenViewsStub());
            else
